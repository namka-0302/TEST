...
const Dashboard: React.FC<DashboardProps> = ({ user, questions, quizzes, accounts, results, onDeleteQuiz }) => {
  ...
  const studentStats = useMemo(() => {
    ...
    // Xu hướng điểm số (5 bài gần nhất)
    ...
    userResults.forEach(res => {
      // Vì QuizResult không lưu breakdown, ta giả định dựa trên score chung hoặc nếu có quiz ID
      const quiz = quizzes.find(q => q.id === res.quizId);
      if (quiz) {
        quiz.questions.forEach(q => {
          difficultyAnalysis[q.difficulty].total++;
          // Logic này chỉ mang tính ước lượng vì QuizResult hiện tại chưa lưu ID câu hỏi đúng/sai chi tiết
          // Trong thực tế sẽ cần update model QuizResult để lưu array questionIds correct
        });
      }
    });

    return { userResults, totalSeen, masteryRate, avgScore, recentScores };
  }, [isAdmin, results, user.id, questions, quizzes]);
    ...
    <div className="space-y-6 md:space-y-8 animate-fadeIn px-1 md:px-0 pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-indigo-900 rounded-[2.5rem] p-8 md:p-10 text-white shadow-2xl relative overflow-hidden flex flex-col justify-between min-h-[240px]">
          <div className="relative z-10">
            ...
            <p className="text-indigo-200 font-medium">Hôm nay bạn muốn chinh phục kiến thức nào?</p>
          </div>
          <div className="relative z-10 flex gap-4 mt-8">
            ...
            <div className="hidden sm:flex items-center gap-2 bg-white/10 px-6 py-4 rounded-2xl border border-white/10 backdrop-blur-md">
               <i className="fas fa-fire text-orange-400"></i>
               <span className="font-black">Chuỗi 3 ngày</span>
            </div>
          </div>
          {/* Decorative Circle */}
        ...
        <div className="md:col-span-2 bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm flex flex-col justify-between">
          ...
          <div className="flex items-end justify-between h-16 gap-2">
            {recentScores.length === 0 ? (
              <p className="text-gray-300 text-[10px] w-full text-center pb-2 italic">Chưa có dữ liệu</p>
            ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          {quizzes.length === 0 ? (
            <div className="bg-white p-12 rounded-[2.5rem] text-center border border-dashed border-gray-200">Chưa có bài thi nào được giao.</div>
          ) : (
        ...
        <div className="space-y-6">
           <h2 className="text-xl font-black text-gray-900">Hoạt động gần đây</h2>
           ...
           {/* Difficulty Stats Mini Card */}
           <div className="bg-gray-900 text-white rounded-[2rem] p-6 shadow-xl">
              <h4 className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-4">Mức độ hoàn thành</h4>
              <div className="space-y-3">
                 {['Easy', 'Medium', 'Hard'].map(diff => {
                    // Logic tính nháp cho UI vì QuizResult chưa có data chi tiết
                    ...
                    return (
                      <div key={diff}>
                        <div className="flex justify-between text-[10px] font-bold mb-1">
                          <span>{labels[diff as keyof typeof labels]}</span>
                          <span>Đã ôn tập</span>
                        </div>
                        <div className="w-full bg-white/10 h-1 rounded-full">
                          <div className={`h-full ${colors[diff as keyof typeof colors]} rounded-full`} style={{ width: diff === 'Easy' ? '85%' : diff === 'Medium' ? '45%' : '15%' }}></div>
                        </div>
                      </div>
                    )
                 })}
