        ...
        <div className="max-w-4xl mx-auto animate-fadeIn space-y-6 pb-20 px-2">
          <div className="bg-white p-4 md:p-6 rounded-3xl shadow-lg border border-gray-100 flex flex-col md:flex-row items-center justify-between sticky top-20 z-50 gap-4">
            <div className="text-center md:text-left">
              <h2 className="text-lg font-black text-gray-900">{isReviewMode ? 'Chi tiết bài đã làm' : 'Xem lại bài thi'}</h2>
              <p className="text-sm text-indigo-600 font-black">Kết quả: {result.score} / {result.totalQuestions}</p>
            </div>
            <div className="flex gap-2 w-full md:w-auto">
              <Link to="/" className="flex-grow md:flex-grow-0 px-6 py-3 bg-gray-100 text-gray-600 rounded-2xl font-black text-sm text-center">Trang chủ</Link>
              {!isReviewMode && (
                <button onClick={() => setShowReview(false)} className="flex-grow md:flex-grow-0 px-6 py-3 bg-gray-900 text-white rounded-2xl font-black text-sm">Kết quả tổng quát</button>
              )}
            </div>
          </div>
                ...
                <div key={q.id} className={`p-6 md:p-8 rounded-[2rem] bg-white border shadow-sm ${isCorrect ? 'border-green-100' : 'border-red-100'}`}>
                  <div className="flex justify-between mb-4">
                    <span className={`text-[9px] font-black uppercase px-2.5 py-1 rounded-full ${isCorrect ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
                      {isCorrect ? 'Chính xác' : 'Sai / Chưa chọn'}
                    </span>
                  ...
                  {q.explanation && (
                    <div className="mt-6 p-5 bg-indigo-50 rounded-2xl border border-indigo-100">
                      <p className="text-[10px] font-black text-indigo-600 uppercase mb-2">Giải thích</p>
    ...
    return (
      <div className="max-w-2xl mx-auto animate-fadeIn px-2">
        <div className="bg-white rounded-[2.5rem] p-8 md:p-12 border border-gray-100 shadow-2xl text-center space-y-8 relative overflow-hidden">
          ...
          <div>
            <h2 className="text-3xl font-black text-gray-900">Hoàn tất!</h2>
            <p className="text-gray-500 mt-2 font-medium">Bạn đã hoàn thành: <span className="text-indigo-600">{quiz.title}</span></p>
          </div>
          ...
          <div className="pt-4 flex flex-col gap-4">
            <button onClick={() => setShowReview(true)} className="w-full bg-white text-indigo-600 border-2 border-indigo-100 py-4 rounded-2xl font-black hover:bg-indigo-50 transition-all">Xem đáp án chi tiết</button>
            <Link to="/" className="w-full bg-indigo-600 text-white py-5 rounded-2xl font-black text-lg hover:bg-indigo-700 shadow-xl shadow-indigo-200">Về Trang chủ</Link>
          </div>
        </div>
      </div>
    );
    ...
    <div className="max-w-4xl mx-auto animate-fadeIn space-y-4 px-1 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-center bg-white p-4 rounded-[2rem] border border-gray-100 shadow-sm sticky top-[4.5rem] z-50 gap-4">
        <div className="text-center md:text-left">
          ...
          <p className="text-[10px] font-black text-gray-400 uppercase">Câu {currentQuestionIdx + 1} / {quiz.questions.length}</p>
        </div>
        <div className="flex items-center gap-4 w-full md:w-auto">
          <div className="flex-grow md:flex-grow-0 flex items-center justify-center gap-3 bg-gray-50 px-5 py-2.5 rounded-2xl border border-gray-100">
            ...
            <span className={`font-mono text-xl font-black ${timeLeft < 60 ? 'text-red-600 animate-pulse' : 'text-gray-700'}`}>{formatTime(timeLeft)}</span>
          </div>
          <button onClick={finishQuiz} className="bg-gray-900 text-white px-8 py-3 rounded-2xl text-sm font-black hover:bg-black shadow-lg">Nộp bài</button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[2.5rem] p-6 md:p-10 border border-gray-50 shadow-xl space-y-10 min-h-[500px] flex flex-col">
        <div className="space-y-4">
          <div className="inline-block px-3 py-1 bg-indigo-50 text-indigo-600 rounded-full text-[10px] font-black uppercase">Câu hỏi {currentQuestionIdx + 1}</div>
          <h3 className="text-xl md:text-2xl font-black text-gray-800 leading-snug">{currentQuestion.text}</h3>
        </div>
          ...
          {currentQuestion.choices.map((choice) => (
            <div 
              ...
              className={`p-5 md:p-6 rounded-2xl border-2 cursor-pointer transition-all flex items-center gap-4 group ${
                answers[currentQuestion.id] === choice.id ? 'border-indigo-600 bg-indigo-50/50' : 'border-gray-50 bg-gray-50 hover:border-gray-200'
              }`}
            >
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-lg shrink-0 transition-colors ${
                answers[currentQuestion.id] === choice.id ? 'bg-indigo-600 text-white shadow-lg' : 'bg-white text-gray-400 border border-gray-100 group-hover:text-indigo-600'
              }`}>{choice.label}</div>
              <span className={`text-base md:text-lg font-bold ${answers[currentQuestion.id] === choice.id ? 'text-indigo-900' : 'text-gray-700'}`}>{choice.text}</span>
            </div>
          ))}
        </div>

        <div className="mt-auto flex justify-between pt-10 border-t border-gray-50">
          <button onClick={() => setCurrentQuestionIdx(Math.max(0, currentQuestionIdx - 1))} disabled={currentQuestionIdx === 0} className="flex items-center gap-2 text-gray-400 font-black hover:text-indigo-600 disabled:opacity-0 transition-all">
            <i className="fas fa-chevron-left"></i> Trước
          </button>
          <button onClick={() => currentQuestionIdx < quiz.questions.length - 1 ? setCurrentQuestionIdx(currentQuestionIdx + 1) : finishQuiz()} className="flex items-center gap-2 bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black hover:bg-indigo-700 shadow-xl transition-all">
            {currentQuestionIdx === quiz.questions.length - 1 ? 'Hoàn tất' : 'Kế tiếp'} <i className="fas fa-chevron-right"></i>
          </button>
        </div>
      </div>

      <div className="flex gap-2 overflow-x-auto pb-4 pt-2 no-scrollbar px-2">
        {quiz.questions.map((_, idx) => (
          <button
            key={idx}
            onClick={() => setCurrentQuestionIdx(idx)}
            className={`w-12 h-12 rounded-2xl font-black text-sm shrink-0 transition-all border-2 ${
              idx === currentQuestionIdx ? 'bg-indigo-600 text-white border-indigo-600 shadow-lg scale-110' : answers[quiz.questions[idx].id] ? 'bg-indigo-50 text-indigo-600 border-indigo-100' : 'bg-white text-gray-400 border-gray-100 hover:border-indigo-200'
            }`}
          >{idx + 1}</button>
        ))}
      </div>
